...
public class FilterConfig
{
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Bean
    public FilterRegistrationBean jimuReportFrameOptionsFilterRegistration()
    {
        FilterRegistrationBean registration = new FilterRegistrationBean();
        registration.setFilter(new com.ruoyi.framework.filter.JimuReportFrameOptionsFilter());
        registration.addUrlPatterns("/jmreport/*");
        registration.setName("jimuReportFrameOptionsFilter");
        // 设置较高的优先级，确保在 Spring Security 的 headers 过滤器之后执行
